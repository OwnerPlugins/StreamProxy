...
echo "Starting StreamProxy installation..."

# Determine plugin path based on architecture
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/StreamProxy
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/StreamProxy
fi
...
cleanup() {
    ...
    [ -d "/tmp/StreamProxy-main" ] && rm -rf "/tmp/StreamProxy-main"
}
...
echo "Downloading StreamProxy..."
wget --no-check-certificate 'https://github.com/OwnerPlugins/StreamProxy/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "Failed to download StreamProxy package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to extract StreamProxy package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/StreamProxy-main/usr/lib/enigma2/python/Plugins/Extensions/StreamProxy" ]; then
    cp -r "$TMPPATH/StreamProxy-main/usr/lib/enigma2/python/Plugins/Extensions/StreamProxy"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/StreamProxy-main/usr/lib64/enigma2/python/Plugins/Extensions/StreamProxy" ]; then
    cp -r "$TMPPATH/StreamProxy-main/usr/lib64/enigma2/python/Plugins/Extensions/StreamProxy"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/StreamProxy-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/StreamProxy-main/usr"/* /usr/ 2>/dev/null
...
#########################################################
#        StreamProxy $version INSTALLED SUCCESSFULLY       #

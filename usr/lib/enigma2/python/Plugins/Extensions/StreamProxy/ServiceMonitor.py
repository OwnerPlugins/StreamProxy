...
try:
    ...
    enhanced_log(
        "✅ External proxy disponibile in ServiceMonitor",
        "INFO",
        "ServiceMonitor")
except ImportError:
    ...
    def is_proxy_esterno_attivo():
        return False
    enhanced_log(
        "⚠️ External proxy non disponibile in ServiceMonitor",
        "WARNING",
        "ServiceMonitor")
...
class StreamProxyServiceMonitor:
    ...
    def _interceptPlayService(
        ...
        try:
            ...
            # Decodifica HTML entities (&amp; → &) che Enigma2 non decodifica
            # con unquote
            ...
            if self._is_already_proxy_url(clean_url):
                ...
                if original_url:
                    ...
                    enhanced_log(
                        "🎬 [SERVICEMONITOR] Creato riferimento m3u8 per gestione HLS", "INFO")
                    return self._call_orig_playService(
            ...
            if EXTERNAL_PROXY_AVAILABLE and is_proxy_esterno_attivo() and (
                ...
                enhanced_log(
                    f"🌐 [SERVICEMONITOR] Proxy esterno attivo, forzo proxy per: {clean_url[:100]}...", "INFO")
                # Non chiamare _should_proxy(), passa direttamente al proxy
                # locale che delegherà all'esterno
            ...
            if any(
                ...
                enhanced_log(
                    "🧹 [SERVICEMONITOR] Pulizia cache per cambio canale VIX", "INFO")
                try:
                ...
                if FREESHOT_AVAILABLE and is_freeshot_link(clean_url):
                    ...
                    try:
                        ...
                        if resolved_freeshot and resolved_freeshot.get(
                            ...
                            enhanced_log(
                                "✅ [SERVICEMONITOR] URL Freeshot proxy creato (supporto fMP4)", "INFO")
                            enhanced_log(
            ...
            try:
                ...
                if TVTAP_WMS_AVAILABLE and is_wms_tvtap_url(clean_url):
                    ...
                    if tvtap_proxy_url:
                        enhanced_log(
                            "✅ [SERVICEMONITOR] URL TVTap WMS risolto", "INFO")
                ...
                # Gestione TVTap standard
                    ...
                    tvtap_proxy_url = f"http://127.0.0.1:7860/proxy/m3u?url={
                        quote(clean_url)}"
                    enhanced_log(
                        "✅ [SERVICEMONITOR] URL TVTap configurato", "INFO")
            except Exception as e:
                enhanced_log(
    ...
    def _should_proxy(self, url: str) -> bool:
        ...
        enhanced_log(
            "🔄 [SERVICEMONITOR] URL non autorizzato, passthrough diretto",
            "DEBUG")
        return False
    ...
    def _force_exteplayer3_for_mpd(self, ref, mpd_url):
        ...
        try:
            ...
            if len(parts) > 0:
                ...
                enhanced_log(
                    "✅ [SERVICEMONITOR] Riferimento modificato per exteplayer3 (5001)",
                    "INFO")

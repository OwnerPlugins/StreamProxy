...
def initConfig():
    ...
    # configProxy.txt settings — seeded from file so Setup shows real values
    _ext_active = str(p.get('attivaProxyEsterno', 'NO')).strip().upper() == 'SI'
    ...
    _usa_ext = str(p.get('usaExtractor', 'SI')).strip().upper() == 'SI'
    config.plugins.streamproxy.usaExtractor = ConfigYesNo(default=_usa_ext)
    _usa_hls = str(p.get('usaHlsProxy', 'SI')).strip().upper() == 'SI'
